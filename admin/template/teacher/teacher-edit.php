	...
	function edit_teacher ($id) {
		...
		if (isset($_POST['edit_teacher'])) {
			...
			if ($image == '') {
				...
				$stmt->bind_param("ssssisssi", $name, $age, $country, $accede, $sex, $experience, $program, $introduce, $id);
				$stmt->execute();
			} else {
				$sql = "UPDATE teacher SET name = ?, age = ?, country = ?, accede = ?, sex = ?, experience = ?, program = ?, introduce = ?, image = ? WHERE id = ?";
				$stmt = $conn_vn->prepare($sql);
				$stmt->bind_param("ssssissssi", $name, $age, $country, $accede, $sex, $experience, $program, $introduce, $image, $id);
				$stmt->execute();
			}
...
<form action="" method="post" enctype="multipart/form-data">
    <div class="rowNodeContentPage">
        ...
        <div class="boxNodeContentPage">
            ...
            <p class="titleRightNCP">Học viên cần chuẩn bị những gì</p>
